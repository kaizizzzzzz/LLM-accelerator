...
int main(int argc, char** argv) {
    ...
    cl::Context context;
    cl::Program program;
    cl::Kernel krnl_mask_softmax_per_head;
    ...
    size_t size_bytes_in1 = sizeof(int) * 2;
    std::vector<int, aligned_allocator<int> > source_in1(2);
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_mask_softmax_per_head = cl::Kernel(program, "mask_softmax_per_head", &err));
            valid_device = true;
    ...
    OCL_CHECK(err, err = krnl_mask_softmax_per_head.setArg(0, buffer_in0));
    OCL_CHECK(err, err = krnl_mask_softmax_per_head.setArg(1, buffer_in1));
    OCL_CHECK(err, err = krnl_mask_softmax_per_head.setArg(2, buffer_out0));
    ...
    // Launch the Kernel
    OCL_CHECK(err, err = q.enqueueTask(krnl_mask_softmax_per_head, nullptr, &event));
    ...
    std::cout << "| " << std::left << std::setw(24) << "mask_softmax_per_head "
