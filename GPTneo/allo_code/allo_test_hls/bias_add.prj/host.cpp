...
int main(int argc, char** argv) {
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_bias_add = cl::Kernel(program, "bias_add", &err));
            valid_device = true;
    ...
    OCL_CHECK(err, err = krnl_bias_add.setArg(0, buffer_in0));
    OCL_CHECK(err, err = krnl_bias_add.setArg(1, buffer_in1));
    OCL_CHECK(err, err = krnl_bias_add.setArg(2, buffer_out0));
    ...
    // Launch the Kernel
    OCL_CHECK(err, err = q.enqueueTask(krnl_bias_add, nullptr, &event));
    ...
    std::cout << "| " << std::left << std::setw(24) << "bias_add "
