...
"pybind passed for prt opt of small GPTneo"
import allo
import numpy as np
def test_pybind11():
    H, L, D, Dffn = 12, 32, 48, 192
    M0, M1 = 4, 4
    Ty = float32
    ...
    mod = allo.IPModule(
        ...
        signature=[ "float32[1536]",
                    ...
                    "int32[2]",
                    "float32[1536]"],
        link_hls="/work/zhang-capra/users/ky427/allo/GPTneo/allo_code/allo_test_hls/GPTneo_small_ptr_verify.prj",
    )
    "generate input"
    ...
    X=X.view('float32').reshape(1536)
    ln1_weight=ln1_weight.view('float32').reshape(48)
    ln1_bias=ln1_bias.view('float32').reshape(48)
    q_proj_weight=q_proj_weight.view('float32').reshape(2304)
    k_proj_weight=k_proj_weight.view('float32').reshape(2304)
    v_proj_weight=v_proj_weight.view('float32').reshape(2304)
    out_proj_weight=out_proj_weight.view('float32').reshape(2304)
    out_proj_bias=out_proj_bias.view('float32').reshape(48)
    ln2_weight=ln2_weight.view('float32').reshape(48)
    ln2_bias=ln2_bias.view('float32').reshape(48)
    mlp_fc_weight=mlp_fc_weight.view('float32').reshape(9216)
    mlp_fc_bias=mlp_fc_bias.view('float32').reshape(192)
    mlp_proj_weight=mlp_proj_weight.view('float32').reshape(9216)
    mlp_proj_bias=mlp_proj_bias.view('float32').reshape(48)
    actual_length=actual_length.view('int32').reshape(2)
    csim_C = np.zeros(1536, dtype=np.float32)
    breakpoint()
    mod(
        X,
        ln1_weight,
