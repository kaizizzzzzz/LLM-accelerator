...
int main(int argc, char** argv) {
    ...
    cl::Context context;
    cl::Program program;
    cl::Kernel krnl_masked_casual_dot_product_attention;
    ...
    std::vector<float, aligned_allocator<float> > source_in0(49152);
    size_t size_bytes_in1 = sizeof(float) * 49152;
    std::vector<float, aligned_allocator<float> > source_in1(49152);
    size_t size_bytes_in2 = sizeof(float) * 49152;
    std::vector<float, aligned_allocator<float> > source_in2(49152);
    size_t size_bytes_in3 = sizeof(int) * 2;
    std::vector<int, aligned_allocator<int> > source_in3(2);
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_masked_casual_dot_product_attention = cl::Kernel(program, "masked_casual_dot_product_attention", &err));
            valid_device = true;
    ...
    OCL_CHECK(err, err = krnl_masked_casual_dot_product_attention.setArg(0, buffer_in0));
    OCL_CHECK(err, err = krnl_masked_casual_dot_product_attention.setArg(1, buffer_in1));
    OCL_CHECK(err, err = krnl_masked_casual_dot_product_attention.setArg(2, buffer_in2));
    OCL_CHECK(err, err = krnl_masked_casual_dot_product_attention.setArg(3, buffer_in3));
    OCL_CHECK(err, err = krnl_masked_casual_dot_product_attention.setArg(4, buffer_out0));
    ...
    // Launch the Kernel
    OCL_CHECK(err, err = q.enqueueTask(krnl_masked_casual_dot_product_attention, nullptr, &event));
    ...
    std::cout << "| " << std::left << std::setw(24) << "masked_casual_dot_product_attention "
